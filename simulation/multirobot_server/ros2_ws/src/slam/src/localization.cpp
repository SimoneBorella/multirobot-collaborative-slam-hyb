...
Localization::Localization(rclcpp::Node::SharedPtr node)
{
    ...
    // Create subscriptions and plot publishers
    for(std::string& robot : robots)
    {
        ...
        scan_subscriptions[robot] = node->create_subscription<sensor_msgs::msg::LaserScan>(
            robot_scan_topic, rclcpp::SensorDataQoS(), [this, robot](sensor_msgs::msg::LaserScan::SharedPtr msg) {
                scan_callback(msg, robot);
        });
        
        landmarks_plot_publishers[robot] = node->create_publisher<visualization_msgs::msg::Marker>(
            "/" + robot + "/landmarks_plot", 10
        );

        scan_plot_publishers[robot] = node->create_publisher<sensor_msgs::msg::LaserScan>(
            "/" + robot + "/scan_plot", rclcpp::SensorDataQoS()
        );
    }
    ...
    for (size_t i = 0; i < robots.size(); ++i)
    {
        ...
        robot_metadata.initial_pose = Pose3(
            Rot3::RzRyRx(0.0, 0.0, initial_poses_array[(i*3)+2]),
            Point3(initial_poses_array[(i*3)], initial_poses_array[(i*3)+1], 0.0)
        );
        robot_metadata.pose_id = 0;
        robot_metadata.landmark_id = 0;
        robot_metadata.map_to_odom = Pose3(Rot3::RzRyRx(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0));
    ...
    for(std::string& robot : robots)
    {
        ...
        // Rotation
        auto q = metadata[robot].initial_pose.rotation().toQuaternion();
...
void Localization::landmarks_callback(const interfaces::msg::PointArray::SharedPtr landmarks_msg, const std::string& robot)
{
    ...
    marker.header.frame_id = landmarks_msg->header.frame_id;
    marker.header.stamp = node->get_clock()->now();
    
    marker.ns = landmarks_msg->header.frame_id;
    marker.id = 0;
    ...
    for (const auto& landmark : landmarks_msg->points) {
    ...
    landmarks_plot_publishers[robot]->publish(marker);
}
...
void Localization::scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr scan_msg, const std::string& robot)
{
    ...
    scan_plot_publishers[robot]->publish(*scan_msg);
}
...
void Localization::publish_map_to_odom()
{
    for(std::string& robot : robots)
    {
        ...
        // Translation
        transform.transform.translation.x = metadata[robot].map_to_odom.translation().x();
        transform.transform.translation.y = metadata[robot].map_to_odom.translation().y();
        transform.transform.translation.z = metadata[robot].map_to_odom.translation().z();
    
        // Rotation
        auto q = metadata[robot].map_to_odom.rotation().toQuaternion();
...
std::optional<Symbol> Localization::nearest_neighbor_data_association(const Point3& observed_point, const std::string& robot)
{
...
std::optional<Symbol> Localization::flann_data_association(const Point3& observed_point, const std::string& robot)
{
...
std::vector<std::pair<Symbol, double>> Localization::probabilistic_data_association(const Point3& observed_point, const std::string& robot)
{
...
void Localization::localization()
{
    try
    {
        ...
        for(std::string& robot : robots)
        {
            ...
            {
                Pose3 odometry = metadata[robot].last_pose.between(pose);
                ...
                new_odom_factors.emplace_shared<BetweenFactor<Pose3>>(metadata[robot].last_x, x, odometry, odometry_noise);

                new_odom_factors.addPrior(x, pose, pose_prior_noise);
            }
            ...
            for (const geometry_msgs::msg::Point &point : transformed_landmarks_points)
            {
                ...
                Point3 observed_landmark(point.x, point.y, point.z);

                Unit3 bearing = pose.bearing(observed_landmark);
                ...
                if(data_association_mode == "nn" || data_association_mode == "flann")
                {
                    ...
                    if (associated_l_opt.has_value())
                    {
                        ...
                        new_obs_factors.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x, associated_l, bearing, range, huber_noise);
                        
                        if (!opt_estimates.exists(associated_l))
                            new_estimates.insert_or_assign(associated_l, graph_estimates.at<Point3>(associated_l));
                    }
                    else
                    {
                        ...
                        new_obs_factors.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x, l, bearing, range, huber_noise);
                    }
                }
                else if (data_association_mode == "pda")
                {
                    ...
                    if (!associations.empty())
                    {
                        for (const auto& [associated_l, probability] : associations)
                        {
                            ...
                            new_obs_factors.emplace_shared<BearingRangeFactor<Pose3, Point3>>(x, associated_l, bearing, range, scaled_noise);
                            if (!opt_estimates.exists(associated_l))
                                new_estimates.insert_or_assign(associated_l, graph_estimates.at<Point3>(associated_l));
                        }
                    }
            ...
            if (window_size != static_cast<size_t>(-1))
            {
                ...
                if (insert_temp_prior)
                {
                    ...
                    if (use_fixed_marginal)
                        new_temp_prior_factors.addPrior(window_x, graph_estimates.at<Pose3>(window_x), marginal_prior_noise);
                    else
                        new_temp_prior_factors.addPrior(window_x, graph_estimates.at<Pose3>(window_x), marginals.marginalCovariance(window_x));
        ...
        if(new_factors.size() > 0 || new_estimates.size() > 0 || remove_factor_indices.size() > 0)
        {
            ...
            for(std::string& robot : robots)
            {
                ...
                {
                    Pose3 correction_odom_pose = metadata[robot].window_odom_poses[metadata[robot].window_odom_poses.size() - lag];
                    Symbol correction_x('x', (metadata[robot].last_x.index() - lag + 1));
                    Pose3 correction_map_pose = opt_estimates.at<Pose3>(correction_x);
    ...
    {
        RCLCPP_ERROR_STREAM(node->get_logger(), "Localization error: " << e.what() <<  " — skipping this iteration.");
        return;
    }
}
