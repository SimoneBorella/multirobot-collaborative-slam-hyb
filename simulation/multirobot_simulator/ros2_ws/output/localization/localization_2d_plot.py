...
def parse_args():
    ...
    parser.add_argument('filename', type=str, default="output/localization/isam_graph_100.txt", help="Input data file")
    parser.add_argument('--show', action='store_true', default=False, help="Show plot")
    parser.add_argument('--variance', action='store_true', default=False, help="Show variance")
...
def parse_pose_line(line):
    ...
    if len(line_split) < 8:
        raise ValueError("Invalid pose line format")

    pose_sym = line_split[1]
    pose_values = [float(v) for v in line_split[2:8]]

    if len(line_split) >= 44:
        covariance_values = [float(v) for v in line_split[8:44]]
    else:
...
def parse_point_line(line):
    ...
    if len(line_split) < 5:
        raise ValueError("Invalid point line format")

    point_sym = line_split[1]
    point_values = [float(v) for v in line_split[2:5]]

    if len(line_split) >= 14:
        covariance_values = [float(v) for v in line_split[5:14]]
    else:
