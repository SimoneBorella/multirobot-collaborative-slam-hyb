 ...
 * @return std::array<double, 2> Cartesian position after transforming wgs84_position using the given wgs84_reference using Mercator projection.
 */
inline std::array<double, 2> to_cartesian(const std::array<double, 2> &wgs84_reference, const std::array<double, 2> &wgs84_position) {
#ifndef M_PI
    ...
    const double ML0{mlfn(wgs84_reference[0] * DEG_TO_RAD)};
    ...
    auto fwd = [&](double lat, double lon) {
        ...
        lon -= wgs84_reference[1] * DEG_TO_RAD;
    ...
    return fwd(wgs84_position[0] * DEG_TO_RAD, wgs84_position[1] * DEG_TO_RAD);
}

/**
 * @param wgs84_reference WGS84 position to be used as reference.
 * @param CartesianPosition Cartesian position to be transformed.
 * @return std::array<double, 2> Approximating a WGS84 position from a given CartesianPosition based on a given wgs84_reference using Mercator projection.
 */
inline std::array<double, 2> from_cartesian(const std::array<double, 2> &wgs84_reference, const std::array<double, 2> &CartesianPosition) {
    ...
    std::array<double, 2> approximatewgs84_position{wgs84_reference};
    std::array<double, 2> cartesianResult{to_cartesian(wgs84_reference, approximatewgs84_position)};
    ...
    while ((d < dPrev) && (d > EPSILON10)) {
        approximatewgs84_position[0] = approximatewgs84_position[0] + signLat * incLat;
        cartesianResult             = to_cartesian(wgs84_reference, approximatewgs84_position);
        dPrev                       = d;
    ...
    while ((d < dPrev) && (d > EPSILON10)) {
        approximatewgs84_position[1] = approximatewgs84_position[1] + signLon * incLon;
        cartesianResult             = to_cartesian(wgs84_reference, approximatewgs84_position);
        dPrev                       = d;
        d                           = std::abs(CartesianPosition[0] - cartesianResult[0]);
    }

    return approximatewgs84_position;
}
