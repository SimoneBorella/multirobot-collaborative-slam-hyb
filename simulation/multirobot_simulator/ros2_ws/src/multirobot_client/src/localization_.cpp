...
namespace multirobot_slam
{
    Localization::Localization()
        : last_state_timestamp_(0.0), t_(0), imu_timestamp_prev_(0.0), odom_first_(true), gnss_first_(true), landmark_id_(0), localization_thread_running_(false)
    {
    }

    Localization::Localization(LocalizationParams &params)
        : params_(params), last_state_timestamp_(0.0), t_(0), imu_timestamp_prev_(0.0), odom_first_(true), gnss_first_(true), landmark_id_(0), localization_thread_running_(false)
    {
    }
    ...
    void Localization::localization()
    {
...

        {
            ...
            if (imu_buffer.empty() || (imu_timestamp_prev_ == 0.0 && imu_buffer.size() < 2))
                return;
        ...
        // Timing
        double t_curr_state = imu_buffer.back().timestamp;
        double dt_state = t_curr_state - last_state_timestamp_;
        ...
        // Update timing
        t_++;
        last_state_timestamp_ = t_curr_state;
