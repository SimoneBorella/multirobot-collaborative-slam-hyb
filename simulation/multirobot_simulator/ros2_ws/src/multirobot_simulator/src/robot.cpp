...
: cmd(0.0, 0.0), environment(environment), prev_linear_vel(0.0), state_update_thread_running(false), node(node)
{
    loadRobotConfiguration(robot_yaml_path);

    odom_publisher = node->create_publisher<nav_msgs::msg::Odometry>(name + "/odom", 10);
    imu_publisher = node->create_publisher<sensor_msgs::msg::Imu>(name + "/imu", 10);
...
Robot::~Robot()
{
    state_update_thread_running.store(false);
    if (state_update_thread.joinable()) {
...
void Robot::loadRobotConfiguration(const std::string& robot_yaml_path) {
    ...
    generator.seed(config["seed"].as<int>());
    ...
    for (const auto& sensor_node : sensor_nodes) {
        ...
        if (type == "imu")
        {
            ...
            linear_bias = sensor_node["linear_bias"].as<double>();
            angular_bias = sensor_node["angular_bias"].as<double>();

            linear_bias_dist = std::normal_distribution<double>(0.0, linear_bias_std_dev);
            angular_bias_dist = std::normal_distribution<double>(0.0, angular_bias_std_dev);
        }
        else if (type == "lidar")
        {
...
{
    if(state_update_thread_running)
        return;

    state_update_thread_running.store(true);

    state_update_thread = std::thread([this, rate]()
                                          {
                ...
                while (state_update_thread_running.load())
                {
                    stateUpdate(rate);
...
void Robot::updateIMU(double rate)
{
    double dt = 1.0 / rate;
    ...
    double linear_vel_noisy = (1 + linear_bias) * cmd.linear_vel + linear_bias;
    double angular_vel_noisy = (1 + angular_bias) * cmd.angular_vel + angular_bias;
    ...
    imu_acceleration = (linear_vel_noisy - prev_linear_vel) / dt;
    imu_angular_vel = angular_vel_noisy;

    prev_linear_vel = linear_vel_noisy;
}
...
void Robot::publishIMU()
{
    sensor_msgs::msg::Imu imu_msg;

    imu_msg.header.stamp = node->get_clock()->now();
    imu_msg.header.frame_id = name + "/base_link";

    // Orientation from imu_position
    tf2::Quaternion q;
    ...
    // Angular velocity
    imu_msg.angular_velocity.z = imu_angular_vel;

    // Linear acceleration
    imu_msg.linear_acceleration.x = imu_acceleration;
