...
struct PosedScan
{
    ...
    PosedScan(Pose2 pose, const sensor_msgs::msg::LaserScan& scan, bool updated = true, bool keep = false)
...
private:
    ...
    std::optional<Symbol> nearest_neighbor_data_association(const Point2& observed_point, const std::string& robot);
    std::optional<Symbol> flann_data_association(const Point2& observed_point, const std::string& robot);
    std::vector<std::pair<Symbol, double>> probabilistic_data_association(const Point2& observed_point, const std::string& robot);
    void localization();
    ...
    std::map<std::string, Pose2> initial_poses;
