...
public:
        ...
        : Node("slam")
    {
        ...
        declare_parameter("pose_prior_noise", std::vector<double>());
    ...
    void setup()
    {
        std::vector<std::string> robots = this->get_parameter("robots").as_string_array();
        
        for (const auto &robot : robots)
        {
            robot_cmd_publishers[robot] = this->create_publisher<geometry_msgs::msg::Twist>("/" + robot + "/cmd_vel", 10);
        }
    ...
    void publishZeroCmd()
    {
        ...
        for(int i=0; i<10; i++)
        {
            for (auto &[robot, cmd_publisher] : robot_cmd_publishers)
            {
