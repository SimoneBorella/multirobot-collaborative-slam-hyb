...
void Mapping::mapping()
{
    ...
    for(std::string& robot : robots)
    {
        ...
        for (PosedScan& posed_scan : posed_scans_[robot])
        {
            ...
            for (size_t i = 0; i < scan.ranges.size(); ++i)
            {
                ...
                double gx = pose.x() + lx * cos(rpy(2)) - ly * sin(rpy(2));
                double gy = pose.y() + lx * sin(rpy(2)) + ly * cos(rpy(2));
    ...
    //             Pose3& pose = last_robot_posed_scan.pose;
...
#ifdef FRONTIER_CELLS_DETECTION_DEBUG
