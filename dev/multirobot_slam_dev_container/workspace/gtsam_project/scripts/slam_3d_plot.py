...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot 3D SLAM results")
    parser.add_argument('--filename', type=str, default="res/slam_3d/after.txt", help="Path to the SLAM result file")
...
def parse_pose_line(line):
    ...
    pose_values = [float(v) for v in line_split[2:8]]  # x, y, z, roll, pitch, yaw
...
def parse_point_line(line):
    ...
    point_values = [float(v) for v in line_split[2:5]]  # x, y, z
...
def main():
    ...
    ax.set_title("3D SLAM Poses and Landmarks with Covariance")

    # Plot poses with their covariance ellipsoids and orientation vectors
    for pose_sym, (pose_values, covariance) in poses.items():
        ...
        ax.text(x + 0.1, y + 0.1, z + 0.1, pose_sym, color='red', fontsize=9)

    # Plot landmarks with their covariance ellipsoids
    ...
    for pose1, pose2 in odometry_edges:
        if pose1 in poses and pose2 in poses:
            ...
            ax.plot([x1, x2], [y1, y2], [z1, z2], color='red', linestyle='--', linewidth=1)
    ...
    for pose, landmark in measure_edges:
        if pose in poses and landmark in landmarks:
            ...
            ax.plot([x1, x2], [y1, y2], [z1, z2], color='blue', linestyle='--', linewidth=1)
