...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot planar SLAM localization results with edges")
...
def main():
    ...
    for pose_sym, (pose_values, covariance) in poses.items():
        pose_sym_text = f"{pose_sym[0]}_{int(int(pose_sym[1:])//1e6)}_{int(int(pose_sym[1:])%1e6)}"
        ...
        if pose_sym in poses_priors:
            ...
            ax.text(x + 0.05, y + 0.05, pose_sym_text, color='green', fontsize=12)
        else:
            ax.scatter(x, y, color='red', marker='o')
            ax.text(x + 0.05, y + 0.05, pose_sym_text, color='red', fontsize=9)
    ...
    for point_sym, (point_values, covariance) in landmarks.items():
        point_sym_text = f"{point_sym[0]}_{int(int(point_sym[1:])//1e6)}_{int(int(point_sym[1:])%1e6)}"
        x, y = point_values
        plot_covariance_ellipse(ax, (x, y), covariance, color='blue', alpha=0.2)
        ax.scatter(x, y, color='blue', marker='x')
        ax.text(x + 0.05, y + 0.05, point_sym_text, color='blue', fontsize=9)
    ...
    for pose, landmark in measure_edges:
        if pose in poses and landmark in landmarks:
            ...
            ax.plot([x1, x2], [y1, y2], color='blue', linestyle='--', linewidth=0.5, alpha=0.2)
