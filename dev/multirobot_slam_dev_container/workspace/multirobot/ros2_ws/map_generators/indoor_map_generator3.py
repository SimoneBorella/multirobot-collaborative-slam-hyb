...
def main(args):
    ...
    room_size = int(args.room_size / resolution)
    hallway_width = int(args.hallway_width / resolution)
    hallway_length = int(args.hallway_length / resolution)
    room_distance = room_size + hallway_length

    rows = args.rows
    cols = args.cols

    # Compute map layout size
    total_width = cols * room_size + (cols - 1) * hallway_length
    total_height = rows * room_size + (rows - 1) * hallway_length

    if total_width > width_grid or total_height > height_grid:
        raise ValueError("Grid too small for given room size and count.")

    occupancy_grid = create_occupancy_grid(width_grid, height_grid, occupancy_value=100)

    rooms = []
    start_y = (height_grid - total_height) // 2
    start_x = (width_grid - total_width) // 2
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            # Carve out the room
            occupancy_grid[top:bottom, left:right] = 0

            # Store room center
            room_center = (top + room_size // 2, left + room_size // 2)
            v_center = room_center[0]
            h_center = room_center[1]

            rooms.append(Room(pos=room_center, width=room_size, height=room_size))

            # Carve horizontal hallway
            if j < cols - 1:
                occupancy_grid[v_center-hallway_width:v_center+hallway_width, h_center:h_center + room_distance] = 0

            # Carve vertical hallway
            if i < rows - 1:
                occupancy_grid[v_center:v_center + room_distance, h_center-hallway_width:h_center+hallway_width] = 0

    # Plot and save
    if args.show:
        plot(occupancy_grid, origin, width_grid, height_grid, resolution, main_rooms=rooms)
    ...
    numbers = [int(re.match(r"indoor_map_(\d+)\.pgm", f).group(1)) for f in files if re.match(r"indoor_map_(\d+)\.pgm", f)]
    map_id = max(numbers) + 1 if numbers else 0

    save_pgm(occupancy_grid, filename=f"{maps_path}/indoor_map_{map_id}.pgm")
    save_yaml(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--width_grid', type=int, default=4000, help='Map width (number of grid cells)')
    parser.add_argument('--height_grid', type=int, default=4000, help='Map height (number of grid cells)')
